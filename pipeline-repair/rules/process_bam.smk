...
rule add_umis_to_bam:
    ...
    input:
      ...
      bai = "{data}/bam/{sample}_haircut_tagged.bam.bai"
    output:
      bam = temp("{data}/bam/{sample}_haircut_umitagged.bam")
    params:
      job_name = "{sample}.group",
      memory = "select[mem>32] rusage[mem=32]",
    log:
      "{data}/logs/group_sample/{sample}"
    threads: 2
...
rule sort_by_cb:
    ...
    """
    input:
      bam = "{data}/bam/{sample}_haircut_umitagged.bam"
    output:
      bam = "{data}/bam/{sample}_haircut_umitagged_sorted.bam"
    params:
      cbtag = "CB" ,
